...
import { api, caps as platformCaps, beforeCapture } from './platform.js';
...
// ---------- action registry ----------
// `icon` names refer to the bundled Phosphor set (see phosphor-icons.js).
...
export function emptyHtml(scope, query, engine) {
  if (scope === 'all')
    return `<div class="cmdk-empty">No results — press Enter to search “${escHtml(query)}” on ${engine}</div>`;
